...
import { useMember, useUser, useUtil } from "@kratercord/common/hooks";
...
export const Account: FC<{ set: Dispatch<SetStateAction<string>>; }> = ({ set }) => {
    ...
    return (
        <div className="relative m-4 mt-0 w-[calc(100%-2rem)] h-[calc(100%-1rem)]">
            ...
            <div className="flex flex-col w-full mt-2 rounded-md h-52 bg-amber-200">
                <div className="w-full overflow-hidden h-28">
                    {banner && <Img className="w-full rounded-t-md" width={600} height={240} src={banner} alt={`${username}:profile-banner`} />}
                </div>
                
                <div className="w-full h-32 bg-gray-900 rounded-b-md">
                    <div className="flex w-full">
                        <div className="relative ml-2 -mt-4 cursor-pointer group">
                            <Img width={68} height={68} className="w-[calc(4rem+4px)] h-[calc(4rem+4px)] border-4 border-gray-900 rounded-full" src={avatar} />
                        </div>
