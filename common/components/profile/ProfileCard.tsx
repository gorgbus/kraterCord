        ...
        <div ref={profileRef} className={`z-50 w-64 rounded-md drop-shadow-xl bg-gray-900 cursor-default ${customClass || "right-[14.5rem] fixed"}`}>
            <div className="flex flex-col w-full h-24 bg-orange-300 rounded-t-md">
                <div className="w-full overflow-hidden h-2/3">
                    {(user?.banner || member?.background || member?.user.background) && <Image className="rounded-t-md" width={600} height={240} src={user?.banner || member?.background || member?.user.background!} alt={`${user?.username}:profile-banner`} />}
                </div>

                <div className="w-full bg-gray-900 h-1/3">
                    <div className="relative ml-4 -mt-10 group">
                        <Image width={80} height={80} className="w-20 h-20 border-4 border-gray-900 rounded-full" alt="avatar-settings" src={user?.avatar || member?.avatar || member?.user.avatar!} />
                    </div>
                </div>
            </div>
